...
export async function injectAssets(html) {
  const blocks = { theme: ['theme.css', 'style'], shared: ['shared.css', 'style'], highlight: ['prd-highlight.js', 'script'], review: ['review.js', 'script'], 'mermaid-style': ['mermaid.css', 'style'], 'mermaid-vendor': ['vendor/mermaid-11.17.2.min.js', 'script'], mermaid: ['mermaid.js', 'script'] };
  for (const [name, [file, tag]] of Object.entries(blocks)) {
    ...
    if (name === 'mermaid-vendor') content = `/* Mermaid 11.17.2\n${await fs.readFile(path.join(assetsDir, 'vendor/mermaid-LICENSE.txt'), 'utf8')}\n*/\n${content}`;
    if (tag === 'script') content = content.replace(/<\/script/gi, '<\\/script');
...
export async function render(context) {
  ...
  for (const g of m.groups) for (const s of g.screens) {
    const contentHash = screenHash(s, g.id, m.requirements, prdDiagrams), previousHash = m.baseline?.[s.id];
  ...
  function renderDiagrams(diagrams, scope, title = '交互与流程图') {
    if (!diagrams.length) return '';
    const heading = scope.startsWith('screen-') ? 'h4' : 'h2';
    return `<section class="diagram-section" aria-label="${e(title)}"><${heading}>${e(title)}</${heading}>${diagrams.map(d => {
      const key = `diagram-${scope}-${d.id}`;
      return `<figure class="mermaid-card" data-mermaid-card id="${e(key)}"><figcaption id="${e(key)}-caption">${e(d.title)}</figcaption>${d.description ? `<p class="diagram-description">${e(d.description)}</p>` : ''}<div class="diagram-tools" role="group" aria-label="${e(d.title)}图示工具"><button type="button" data-diagram-zoom="out" disabled aria-label="缩小图示">−</button><button type="button" data-diagram-zoom="in" disabled aria-label="放大图示">＋</button><button type="button" data-diagram-zoom="fit" disabled>适应宽度</button><button type="button" data-diagram-zoom="actual" disabled>原始大小</button><span class="diagram-scale"></span><button type="button" data-diagram-copy>复制源码</button></div><div class="diagram-view" tabindex="0" role="region" aria-labelledby="${e(key)}-caption"><div class="diagram-stage"></div></div><p class="diagram-hint"></p><p class="diagram-status" role="status" aria-live="polite">图示可视化需启用 JavaScript；下方保留源码。</p><details class="diagram-source" open><summary>查看 Mermaid 源码</summary><pre><code>${e(d.source)}</code></pre></details>${d.line ? `<p class="diagram-origin">${scope === 'prd' ? `PRD 第 ${d.line} 行 · ${e(d.id)}` : `<a href="index.html#diagram-prd-${e(d.id)}">引用 PRD 图示 · ${e(d.id)}</a>`}</p>` : ''}</figure>`;
    }).join('')}</section>`;
  }
  ...
  const shell = (title, current, main) => {
    const hasDiagrams = main.includes('class="mermaid-card"');
    return `<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><meta http-equiv="Content-Security-Policy" content="default-src 'none'; img-src data: blob:; style-src 'unsafe-inline'; script-src 'unsafe-inline'; connect-src http://127.0.0.1:9224; base-uri 'none'; form-action 'none'"><title>${e(title)}</title>${marker('theme')}${marker('shared')}${hasDiagrams ? marker('mermaid-style') : ''}</head><body><div class="workspace">${sidebar(current)}<main>${main}</main></div><script id="pm-data" type="application/json">${json({ ...model, page: current })}</script><script id="pm-feedback" type="application/json">{}</script>${marker('highlight')}${marker('review')}${hasDiagrams ? marker('mermaid-vendor') + marker('mermaid') : ''}</body></html>`;
  };
